...
struct Address: SwiftProtobuf.Message {
  ...
  mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    while let fieldNumber = try decoder.nextFieldNumber() {
      ...
      case 2: try decoder.decodeSingularStringField(value: &self.postcode)
      default: break
      }
    }
  }
  ...
  func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    ...
    if !self.postcode.isEmpty {
      try visitor.visitSingularStringField(value: self.postcode, fieldNumber: 2)
    }
    try unknownFields.traverse(visitor: &visitor)
  }
}
...
extension Address: SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  func _protobuf_generated_isEqualTo(other: Address) -> Bool {
    ...
    if self.postcode != other.postcode {return false}
    if unknownFields != other.unknownFields {return false}
    return true
  }
}
